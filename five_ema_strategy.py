...
class StrategyConfig:
    ...
    # --- Strategy Parameters ---
    base_target: float = 20.0            # Base profit target per trade (points)
...
class BacktestEngine:
    ...
    def run(self, date, symbol: str = None) -> pd.DataFrame:
        ...
        print(f"BACKTESTING: {date}  |  Symbol: {symbol}  |  "
              f"Base Target: {self.config.base_target} pts  |  "
        ...
        # =================================================================
        target_points = self.config.base_target   # dynamic, shared across sides
        cumulative_loss = 0.0
        cumulative_pnl = 0.0
            ...
            for _, tick in ticks.iterrows():
                ...
                for side in list(open_positions.keys()):
                    ...
                    if pnl < 0:
                        ...
                        target_points = self.config.base_target + cumulative_loss
                    else:
                        cumulative_loss = max(0.0, cumulative_loss - pnl)
                        target_points = max(self.config.base_target,
                                            self.config.base_target + cumulative_loss)
                    cumulative_pnl += pnl
                    ...
                    print(f"  >> {exit_type} {side} @ {tick_time} | "
                          ...
                          f"Target Pts: {target_points:.1f}")

                    if (self.config.daily_target is not None
                    ...
                    if can_enter:
                        ...
                        if entered:
                            ...
                            print(f"\n  #{trade_counter} {sig_side} Entry @ {tick_time}"
                                  ...
                                  f" | Target: {tp:.2f} ({target_points:.1f} pts)"
